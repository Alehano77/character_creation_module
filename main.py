...
def defencedefence(char_name, char_class):
    if char_class == 'warrior':
...
def start_training(char_name, char_class):
    ...
    cmd = None
    while char_class != 'skip':
        cmd = input('Введи команду: ')
        if char_class == 'attack':
            print(attack(char_name, char_class))
        if char_class == 'defence':
            print(defence(char_name, char_class))
        if char_class == 'special':
...
def choice_char_class():
    ...
    while char_class != 'y':
        ...
        if char_class == 'mage':
            printprint('Маг — находчивый воин дальнего боя. Обладает высоким интеллектом.')
        if char_class == 'healer':
